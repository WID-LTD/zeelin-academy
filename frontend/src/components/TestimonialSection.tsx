...
function StarRating() {
  return (
    <div className="flex items-center gap-1">
      {[...Array(5)].map((_, i) => (
        <svg key={i} className="w-4 h-4 3xl:w-5 3xl:h-5" fill="currentColor" viewBox="0 0 20 20" style={{ color: 'var(--brand-gold)' }}>
  ...
  return (
    <section className="relative overflow-hidden py-16 md:py-24 3xl:py-28" style={{ backgroundColor: 'var(--bg-secondary)' }}>
      <div className="max-w-[2560px] mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 3xl:gap-8 max-w-[1200px] 3xl:max-w-[1600px] mx-auto">
          {testimonials.map((item, idx) => (
            <AnimatedSection key={item.name} delay={100 + idx * 80} duration={600}>
              <div
                className="p-6 3xl:p-8 border transition-all duration-300 h-full flex flex-col hover:shadow-lg hover:border-[rgba(212,175,55,0.3)] group"
                style={{ backgroundColor: 'var(--bg-card)', borderColor: 'var(--border)' }}
              >
                <div className="flex items-start gap-4 3xl:gap-5 mb-5">
                  <div
                    className="w-10 h-10 3xl:w-12 3xl:h-12 flex items-center justify-center flex-shrink-0 transition-all duration-300 group-hover:scale-110"
                    style={{ backgroundColor: 'rgba(212,175,55,0.1)' }}
                  >
                    <svg className="w-5 h-5 3xl:w-6 3xl:h-6" fill="currentColor" viewBox="0 0 24 24" style={{ color: 'var(--brand-gold)' }}>
                      <path d="M14.017 21v-7.391c0-5.704 3.731-9.57 8.983-10.609l.995 2.151c-2.432.917-3.995 3.638-3.995 5.849h4v10h-9.983zm-14.017 0v-7.391c0-5.704 3.748-9.57 9-10.609l.996 2.151c-2.433.917-3.996 3.638-3.996 5.849h3.983v10h-9.983z" />
                    </svg>
                  </div>
                  <div>
                    <h4 className="font-display font-bold text-lg 3xl:text-xl" style={{ color: 'var(--text-core)' }}>
                      {item.name}
                    </h4>
                    <p className="text-xs 3xl:text-sm font-semibold mt-0.5" style={{ color: 'var(--brand-gold)' }}>
                      {item.role}
                    </p>
                  </div>
                </div>

                <p className="text-sm 3xl:text-base leading-relaxed flex-1 mb-5" style={{ color: 'var(--text-secondary)' }}>
