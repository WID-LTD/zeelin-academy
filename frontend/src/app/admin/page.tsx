...
import { clearAdminAuth, getAdminToken, getAdminUser, dispatchAuthEvent } from '@/lib/auth'
...
export default function AdminDashboard() {
  ...
  const handleGenerateCredentials = async (enrollment: Enrollment) => {
    const data = await adminApi.post<{ generatedPassword?: string }>('/api/auth/admin/generate-credentials', {
      enrollment_id: enrollment.id, email: enrollment.email, full_name: enrollment.full_name,
    })
  ...
  const handleChangePassword = async (userId: number) => {
    ...
    await adminApi.post('/api/auth/admin/change-password', { user_id: userId, password: newPassword })
  ...
  return (
    <div className="min-h-screen pt-20 flex">
      {/* Sidebar */}
      <aside className="w-64 shrink-0 border-r border-[color:var(--border)] bg-[color:var(--bg-card)] hidden lg:flex flex-col">
      ...
      {/* Main content */}
      <div className="flex-1 py-8 px-4 sm:px-6 lg:px-8 min-w-0">
        {/* Mobile tabs */}
        ...
        <div className="flex items-center justify-between mb-8">
          ...
          <div className="flex items-center gap-3">
            ...
            <Link href="/" className="text-sm text-muted hover:gold">&larr; Site</Link>
          </div>
        </div>
        ...
        {/* Back link */}
        <div className="mt-6"><Link href="/" className="text-sm footer-link">&larr; Back to Home</Link></div>
      </div>
    </div>
  )
}
